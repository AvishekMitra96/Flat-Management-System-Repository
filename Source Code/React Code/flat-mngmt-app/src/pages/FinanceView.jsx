      ...
      <div className="glass-panel p-6">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-slate-300">
            ...
            <tbody>
              {dummyDues.map((due, idx) => (
                <tr key={idx} className="border-b border-slate-700/50 hover:bg-slate-800/50 transition-colors">
                  <td className="px-6 py-4 font-medium text-slate-200">{due.flat}</td>
                  <td className="px-6 py-4">{due.amount}</td>
                  <td className="px-6 py-4">{due.dueDate}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2.5 py-1 text-xs font-semibold rounded-full ${
                      due.status === 'Paid' ? 'bg-emerald-500/20 text-emerald-400' :
                      due.status === 'Pending' ? 'bg-amber-500/20 text-amber-400' :
          ...
          {dummyExpenses.map((exp, idx) => (
            <div key={idx} className="bg-slate-900/50 border border-slate-700 rounded-xl p-5 relative">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <span className="text-xs text-slate-500 font-mono">{exp.id}</span>
                  <h4 className="text-lg font-semibold text-slate-200 mt-1">{exp.merchant}</h4>
                </div>
